...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
	...
	CREATE ROLE berean_owner LOGIN PASSWORD '${BEREAN_DB_OWNER_PASSWORD}';

	-- Writes corpus tables. No access to trace tables.
	CREATE ROLE catena LOGIN PASSWORD '${BEREAN_DB_CATENA_PASSWORD}';

	-- Writes session and trace tables. Read-only on corpus tables.
	CREATE ROLE gateway LOGIN PASSWORD '${BEREAN_DB_GATEWAY_PASSWORD}';
